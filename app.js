...
function defaultData(){return{tasks:[],xp:0,totalCompleted:0,streak:0,maxStreak:0,badges:[],customBadges:[],dailyLog:{},lastActive:null,profile:{name:'我',avatar:'🐱'},dock:['today','calendar','history','badges','goals'],routines:[],goals:[],goalLogs:{},lastRoutineGen:null,theme:'purple',createdAt:today(),trackStats:{},customCats:[],_v:3}}
function mergeData(d){
  const def=defaultData();
  return{...def,...d,profile:{...def.profile,...(d.profile||{})},dock:d.dock||def.dock,routines:d.routines||[],goals:d.goals||[],goalLogs:d.goalLogs||{},lastRoutineGen:d.lastRoutineGen||null,theme:d.theme||'purple',trackStats:d.trackStats||{},customCats:d.customCats||[],_v:3};
}
...
let _calOpenDs=null;
function renderAll(){
  renderHeader();renderSidebar();renderToday();renderTimeline();renderCalendar();renderHistory();renderBadges();renderStats();renderGoals();renderTracker();
...
// ==================== TRACKER (custom categories + period stats) ====================
function getAllCats(){
  // Merge built-in CATS with customCats
  const builtin=CATS.map(c=>({id:c.id,label:c.label,icon:c.icon,color:c.c,unit:'',custom:false}));
  const custom=(ud.customCats||[]).map(c=>({id:c.id,label:c.label,icon:c.icon||'📌',color:c.color||'#8B5CF6',unit:c.unit||'',custom:true}));
  return builtin.concat(custom);
}
function renderTracker(){
  renderTrackerPeriods();
  renderTrackerCats();
}
function renderTrackerPeriods(){
  const container=document.getElementById('trackerPeriodStats');
  if(!container)return;
  const now=new Date();
  // This week (Mon-Sun)
  ...
  const ws=weekStart.getTime(),ms=monthStart.getTime(),ys=yearStart.getTime();
  let wCount=0,mCount=0,yCount=0,allCount=0;
  ud.tasks.forEach(t=>{
  ...
  const items=[
    {label:'本周',val:wCount,icon:'📅'},
    {label:'本月',val:mCount,icon:'🗓️'},
    {label:'本年',val:yCount,icon:'🏆'},
    {label:'累计',val:allCount,icon:'✅'},
  ];
  container.innerHTML=items.map(i=>
    '<div class="bg-[#18181B] rounded-xl p-4 text-center">'+
    '<div class="text-2xl mb-1">'+i.icon+'</div>'+
    '<div class="text-2xl font-bold" style="color:var(--ac)">'+i.val+'</div>'+
    '<div class="text-xs text-gray-500 mt-0.5">'+i.label+'完成</div></div>'
  ).join('');
}
function renderTrackerCats(){
  const container=document.getElementById('trackerCatGrid');
  if(!container)return;
  ...
  let html='';
  cats.forEach(c=>{
    const completed=ud.tasks.filter(t=>t.completed&&t.category===c.id);
    const weekCount=completed.filter(t=>t.completedAt&&new Date(t.completedAt).getTime()>=weekStart.getTime()).length;
    const lastDate=completed.length>0?completed.sort((a,b)=>new Date(b.completedAt)-new Date(a.completedAt))[0].completedAt:null;
    const lastStr=lastDate?lastDate.slice(0,10):'无';
    html+='<div class="bg-[#18181B] rounded-xl p-4 cursor-pointer hover:bg-[#27272A] transition-colors" onclick="showTrackerCatDetail(\''+c.id+'\')">'+
      '<div class="flex items-center gap-3 mb-2">'+
        '<span class="text-2xl">'+c.icon+'</span>'+
        '<div class="flex-1"><div class="text-sm font-bold">'+c.label+'</div>'+
        '<div class="text-xs text-gray-500">'+(c.custom?'自定义':'内置')+(c.unit?' · 单位:'+c.unit:'')+'</div></div>'+
        '<div class="text-right"><div class="text-lg font-bold" style="color:'+c.color+'">'+completed.length+'</div>'+
        '<div class="text-[10px] text-gray-500">总完成</div></div>'+
      '</div>'+
      '<div class="flex items-center justify-between text-[10px] text-gray-500">'+
        '<span>本周 '+weekCount+'</span>'+
        '<span>最近: '+lastStr+'</span>'+
      '</div></div>';
  });
  container.innerHTML=html;
}
function showTrackerCatDetail(catId){
  const cats=getAllCats();
  const c=cats.find(x=>x.id===catId);if(!c)return;
  const tasks=ud.tasks.filter(t=>t.completed&&t.category===catId);
  // Group by date
  const byDate={};
  tasks.forEach(t=>{
    const d=t.completedAt?t.completedAt.slice(0,10):(t.createdAt||'未知');
  ...
  tasks.forEach(t=>{
    ...
    const ts=new Date(t.completedAt).getTime();
    if(ts>=weekStart)wc++;if(ts>=monthStart)mc++;if(ts>=yearStart)yc++;
  });
  ...
  let html='<div class="flex items-center gap-3 mb-5">'+
    '<button onclick="closeTrackerDetail()" class="text-gray-500 text-sm">← 返回</button>'+
    '<span class="text-2xl">'+c.icon+'</span>'+
    '<h1 class="text-lg font-bold flex-1">'+c.label+'</h1>';
  if(c.custom)html+='<button onclick="delCustomCat(\''+c.id+'\')" class="text-xs text-red-400">删除分类</button>';
  html+='</div>';
  // Period stats
  html+='<div class="grid grid-cols-4 gap-2 mb-5">'+
    '<div class="bg-[#18181B] rounded-lg p-3 text-center"><div class="text-lg font-bold" style="color:var(--ac)">'+wc+'</div><div class="text-[10px] text-gray-500">本周</div></div>'+
    '<div class="bg-[#18181B] rounded-lg p-3 text-center"><div class="text-lg font-bold" style="color:var(--ac)">'+mc+'</div><div class="text-[10px] text-gray-500">本月</div></div>'+
    '<div class="bg-[#18181B] rounded-lg p-3 text-center"><div class="text-lg font-bold" style="color:var(--ac)">'+yc+'</div><div class="text-[10px] text-gray-500">本年</div></div>'+
    '<div class="bg-[#18181B] rounded-lg p-3 text-center"><div class="text-lg font-bold" style="color:var(--ac)">'+tasks.length+'</div><div class="text-[10px] text-gray-500">累计</div></div>'+
    '</div>';
  if(c.unit){
    // Sum extracted values for this category
    let total=0;
    tasks.forEach(t=>{
      const extracted=extractTrack(t.title||'');
      extracted.forEach(e=>{if(e.key===c.id)total+=e.val});
    });
    if(total>0)html+='<div class="bg-[#18181B] rounded-xl p-4 mb-5 text-center"><div class="text-2xl font-bold" style="color:'+c.color+'">'+total+'</div><div class="text-xs text-gray-500">'+c.label+'累计（'+c.unit+'）</div></div>';
  }
  // Task list by date
  if(dates.length===0){
    html+='<div class="text-center py-12 text-gray-500 text-sm">还没有完成过该分类的任务</div>';
  }else{
    html+='<div class="space-y-3">';
    dates.forEach(d=>{
      const dayTasks=byDate[d];
      html+='<div><div class="text-xs text-gray-500 mb-1.5 sticky top-0 bg-[#09090B] py-1">'+d+'（'+dayTasks.length+'个）</div>';
      html+='<div class="space-y-1">';
      dayTasks.forEach(t=>{
        html+='<div class="flex items-center gap-2 bg-[#18181B] rounded-lg px-3 py-2">'+
          '<span class="text-green-400 text-xs">✓</span>'+
          '<span class="text-sm text-gray-300 flex-1 truncate">'+(t.title||'')+'</span>'+
          '</div>';
      });
      html+='</div></div>';
    });
...
function showAddCatModal(){
  const m=document.getElementById('addCatModal');
  if(!m)return;
  // Reset form
  document.getElementById('ccName').value='';
  document.getElementById('ccIcon').value='';
  document.getElementById('ccUnit').value='';
  document.getElementById('ccColor').value='#8B5CF6';
  // Render color buttons
  const cc=document.getElementById('ccColors');
  if(cc){
    cc.innerHTML=['#8B5CF6','#3B82F6','#F59E0B','#10B981','#EC4899','#F97316','#14B8A6','#6366F1'].map((col,i)=>
      '<button type="button" onclick="document.getElementById(\'ccColor\').value=\''+col+'\';document.querySelectorAll(\'#ccColors button\').forEach(b=>b.style.borderColor=\'#3F3F46\');this.style.borderColor=\''+col+'\'" class="w-8 h-8 rounded-full border-2 '+(i===0?'border-[#8B5CF6]':'border-[#3F3F46]')+'" style="background:'+col+'"></button>'
    ).join('');
  }
  m.classList.remove('hidden');m.classList.add('flex');
}
function closeAddCatModal(){const m=document.getElementById('addCatModal');if(m){m.classList.add('hidden');m.classList.remove('flex')}}
function saveCustomCat(){
  const name=document.getElementById('ccName').value.trim();
  const icon=document.getElementById('ccIcon').value.trim()||'📌';
  const unit=document.getElementById('ccUnit').value.trim();
  const color=document.getElementById('ccColor').value||'#8B5CF6';
  if(!name){alert('请填写分类名称');return}
  const id='custom_'+Date.now();
  if(!ud.customCats)ud.customCats=[];
  ud.customCats.push({id,label:name,icon,unit,color});
  saveData();
  closeAddCatModal();
  renderTracker();
}
function delCustomCat(id){
  if(!confirm('删除该分类？已完成任务不会被删除，但会失去分类关联。'))return;
  ud.customCats=(ud.customCats||[]).filter(c=>c.id!==id);
  saveData();
  closeTrackerDetail();
  renderTracker();
}
...
function setView(v){
  ...
  const titles={today:'今日任务',calendar:'日历',timeline:'时间轴',history:'历史任务',badges:'成就徽章',stats:'数据统计',goals:'年度目标',tracker:'分类追踪'};
...
function showCat(title){
  ...
  document.getElementById('catOpts').innerHTML=getAllCats().map(c=>'<button onclick="confirmCat(\''+c.id+'\')" class="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm text-white hover:bg-[#27272A] transition-colors"><span>'+c.icon+'</span> '+c.label+(c.custom?' <span class="text-[10px] text-gray-600">自定义</span>':'')+'</button>').join('');
...
let _pfSelAva=null;
const DOCK_ITEMS=[{id:'today',label:'任务'},{id:'calendar',label:'日历'},{id:'history',label:'历史'},{id:'badges',label:'徽章'},{id:'goals',label:'目标'},{id:'timeline',label:'时间轴'},{id:'tracker',label:'追踪'},{id:'stats',label:'统计'}];
